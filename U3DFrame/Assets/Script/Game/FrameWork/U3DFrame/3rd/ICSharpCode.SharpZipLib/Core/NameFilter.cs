		...
		void Compile()
		{
			...
			for (int i = 0; i < items.Length; ++i) {
				if ((items[i] != null) && (items[i].Length > 0)) {
					...
					if (include) {
						inclusions_.Add(new Regex(toCompile, (RegexOptions)(RegexOptions.IgnoreCase | RegexOptions.Singleline)));
					} else {
                        exclusions_.Add(new Regex(toCompile, (RegexOptions)(RegexOptions.IgnoreCase | RegexOptions.Singleline)));
					}
				}
			}
		}
